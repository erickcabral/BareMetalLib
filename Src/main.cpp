...
void EXTI0_IRQHandler(void) {
	...
	//resetEXTIPendingRegister(PIN_13, extiCFG);
	bool test = true;
...
int main(void) {
	...
	setGPIO_IRQ(40, true);
	setIRQ_PRIORITY(40, 1);

	while (1) {
		if (gpioC.getPinIDR(PIN_13)) {
			setNVIC_PendingBit(40, true);
			//EXTI0_IRQHandler();
			//setEXTIPendingRegister(6, extiCFG);
			//extiCFG.setPR(ENABLE, 6);
		} else {
